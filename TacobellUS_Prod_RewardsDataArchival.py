#!/usr/bin/env python3
# coding: utf-8
## Enhanced TacobellUS_Prod_RewardsDataArchival.py - Final Version 2
## With optional archival insert and clean logging
...
def execute_monitoring_query_direct(query, params=None, fetch=False):
    """Execute query on monitoring database directly (for signal handlers)"""
    connection = None
    cursor = None
    try:
        if not monitoring_connection_config:
            return None
        ...
        connection = mysql.connector.connect(
            ...
            autocommit=True  # Important for signal handlers
        )
        ...
        print(f"Signal handler database error: {str(e)}")
        return None
    finally:
        if cursor:
            cursor.close()
...
def signal_handler(signum, frame):
    ...
    shutdown_reason = f"Process terminated by {signal_name} signal (PID: {process_pid})"
    print(f"\n{shutdown_reason}")
    
    try:
        # Update process_logs status
        if process_log_id:
            update_query = """
            UPDATE process_logs 
            SET end_time = %s, status = 'KILLED', error_message = %s, 
                current_file = %s, current_batch = %s
            WHERE id = %s
            """
            execute_monitoring_query_direct(update_query, (
                datetime.now(), shutdown_reason, current_processing_file, 
                current_processing_batch, process_log_id
            ))
            print(f"Updated process_logs status to KILLED for process {process_log_id}")
        
        # Update file_processing_logs status
        if file_processing_log_id:
            file_update_query = """
            UPDATE file_processing_logs 
            SET end_time = %s, status = 'KILLED', error_message = %s, 
                records_processed = %s, last_processed_batch = %s
            WHERE id = %s
            """
            # Calculate last processed batch (current - 1 since current wasn't completed)
            last_batch = max(0, (current_processing_batch or 1) - 1)
            execute_monitoring_query_direct(file_update_query, (
                datetime.now(), shutdown_reason, total_records_processed, 
                last_batch, file_processing_log_id
            ))
            print(f"Updated file_processing_logs status to KILLED for file log {file_processing_log_id}")
        ...
        try:
            ...
            with open(log_file, 'a') as f:
                f.write(f"[{datetime.now()}] {shutdown_reason}\n")
        except:
            pass
            
    except Exception as e:
        print(f"Error updating database during shutdown: {e}")
...
def execute_monitoring_query(query, params=None, fetch=False):
    ...
    try:
        ...
        if connection:
            connection.rollback()
        # Removed the WARNING print to clean up logs
        return None
    finally:
        if cursor:
            cursor.close()
...
def create_process_log_entry(table_name, process_type='DATA_ARCHIVAL'):
    ...
    if process_log_id:
        print(f"Created process log entry with ID: {process_log_id}")
...
def create_file_processing_log(file_name, file_size):
    ...
    if file_processing_log_id:
        print(f"Created file processing log entry with ID: {file_processing_log_id}")
...
def update_file_processing_log(status, error_message=None, total_batches=0, processed_batches=0, last_processed_batch=0, records_processed=0):
    """Update file processing log entry with correct parameter mapping"""
    if not file_processing_log_id:
        return
        
    # Only set end_time when moving to a terminal state; keep NULL during PROCESSING
    terminal_states = {'COMPLETED', 'FAILED', 'KILLED', 'TERMINATED', 'SYSTEM_REBOOT'}
    end_time_value = datetime.now() if status in terminal_states else None
    ...
    params = (
        ...
        file_processing_log_id  # WHERE id
    )
    
    result = execute_monitoring_query(query, params)
    # Removed the WARNING message - just silently handle the result
    if result is not None:
        print(f"Updated file processing log {file_processing_log_id}: status={status}, records={records_processed}")
...
def update_process_log(status, error_message=None, replica_lag=0, hll_value=0):
    ...
    params = (
        ...
        current_processing_batch, # current_batch
        replica_lag,            # replica_lag
        hll_value,              # hll_value
        process_log_id          # WHERE id
    )
    
    result = execute_monitoring_query(query, params)
    if result is not None:
        print(f"Updated process log {process_log_id}: status={status}")
    return result
...
def update_process_file_counts(total_files=None, processed_files=None):
    ...
    if total_files is not None:
        ...
        if result is not None:
            print(f"Updated total_files to {total_files}")
    ...
    if processed_files is not None:
        ...
        if result is not None:
            print(f"Updated processed_files to {processed_files}")
...
def run_sql_queries(host, user, password, database, queries, fetch_one=False, dictionary=False, multi=False):
    ...
    try:
        ...
        cursor.execute(queries, multi=multi)
...
def get_health_metrics(host, user, password, database, replica_hosts):
    ...
    if replica_hosts:
        ...
        for replica_host in replica_hosts:
            try:
                ...
                slavest = run_sql_queries(replica_host, user, password, database, slave_status, fetch_one=True, dictionary=True)
                if slavest:
                    # MySQL 8 uses Seconds_Behind_Source; older uses Seconds_Behind_Master
                    replication_lag_sec = slavest.get('Seconds_Behind_Source') or slavest.get('Seconds_Behind_Master') or 0
                ...
                get_current_hll = run_sql_queries(replica_host, user, password, database, hll_query, fetch_one=True, dictionary=True)
                if get_current_hll:
                    value = get_current_hll.get('count') if isinstance(get_current_hll, dict) else (get_current_hll[0] if len(get_current_hll) > 0 else 0)
                    hll_all.append(value)
                    print(f"Replica {replica_host} HLL: {value}")
            except Exception as e:
    ...
    try:
        hll_query = "SELECT count FROM information_schema.INNODB_METRICS WHERE name LIKE '%trx_rseg_history_len%'"
        get_current_hll = run_sql_queries(host, user, password, database, hll_query, fetch_one=True, dictionary=True)
        if get_current_hll:
            value = get_current_hll.get('count') if isinstance(get_current_hll, dict) else (get_current_hll[0] if len(get_current_hll) > 0 else 0)
            hll_all.append(value)
            print(f"Master {host} HLL: {value}")
    except Exception as e:
    ...
    try:
        ...
        while True:
            if continuation_token:
                response = s3.list_objects_v2(Bucket=bucket_name, Prefix=file_path, ContinuationToken=continuation_token)
            else:
...
def determine_processing_mode(config):
    """Determine processing mode based on configuration"""
    has_archival = config.has_option('database', 'archival_insert_query')
    has_delete = config.has_option('database', 'delete_query')
    
    if not has_delete:
        raise ProcessingError("delete_query is required in configuration")
    
    if has_archival:
        archival_query = config.get('database', 'archival_insert_query').strip()
        if archival_query:
            return 'ARCHIVE_THEN_DELETE', archival_query
...
def process_file_batches(s3, bucket_name, filename, batch_size, config, db_config, replica_hosts, log_files):
    """Process file in batches with flexible archival/delete modes"""
    ...
    try:
        ...
        processing_mode, archival_query_template = determine_processing_mode(config)
        delete_query_template = config.get('database', 'delete_query')
        
        print(f"Processing mode: {processing_mode}")
        if processing_mode == 'ARCHIVE_THEN_DELETE':
            print("Will archive data first, then delete")
        else:
            print("Will directly delete data (no archival)")
        ...
        if start_batch > 0:
            ...
            with open(log_files['main'], 'a') as f:
                f.write(f"STARTING: {filename} (Size: {file_size} bytes) - Mode: {processing_mode}\n")
        ...
        for i, chunk in enumerate(chunks):
            ...
            if graceful_shutdown:
                # Use a safe fallback for last processed batch if current is not set yet
                last_batch_value = max(0, (current_processing_batch or start_batch or 1) - 1)
                ...
                update_file_processing_log('KILLED', error_msg, total_batches, processed_batches, 
                                         last_batch_value, total_records_processed)
                return False
            ...
            try:
                ...
                if processing_mode == 'ARCHIVE_THEN_DELETE':
                    ...
                    print(f"Archive and delete completed for batch {current_processing_batch} ({record_count} records)")
                    
                else:  # DIRECT_DELETE mode
                    # Execute delete query only
                    delete_query = delete_query_template.format(comma_separated_values)
                    run_sql_queries(db_config['host'], db_config['user'], db_config['password'], 
                                   db_config['database'], delete_query)
                    print(f"Direct delete completed for batch {current_processing_batch} ({record_count} records)")
                ...
                update_file_processing_log('FAILED', error_msg, total_batches, processed_batches, 
                                         max(0, current_processing_batch - 1), total_records_processed)
                raise ProcessingError(error_msg)
        ...
        with open(log_files['main'], 'a') as f:
            f.write(f"FILE PROCESSED SUCCESSFUL: {filename} (Batches: {processed_batches}/{total_batches}, Records: {total_records_processed}) - Mode: {processing_mode}\n")
        ...
        print(error_msg)
        update_file_processing_log('FAILED', error_msg, 0, 0, 0, total_records_processed)
        with open(log_files['main'], 'a') as f:
...
def main():
    ...
    try:
        ...
        if graceful_shutdown:
            final_status = 'KILLED'
            final_error_msg = shutdown_reason or f"Process terminated gracefully. Processed {processed_count} files, {failed_count} failed"
        elif failed_count == 0:
